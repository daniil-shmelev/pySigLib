...
def test_log_sig_combine_cuda_random(deg):
    dim = 5
    ...
    ls_combined = pysiglib.log_sig_combine(ls1.cuda(), ls2.cuda(), dim, deg)
    check_close(ls_expected, ls_combined)


@skip_no_cuda
@pytest.mark.parametrize("deg", range(1, 6))
def test_log_sig_combine_cuda_random_batch(deg):
    dim = 5
...
@skip_no_cuda
@pytest.mark.parametrize("deg", range(1, 6))
def test_log_sig_combine_cuda_identity(deg):
    dim = 5
    ...
    ls_combined = pysiglib.log_sig_combine(ls.cuda(), zero.cuda(), dim, deg)
    check_close(ls, ls_combined)


@skip_no_cuda
@pytest.mark.parametrize("deg", range(2, 6))
def test_log_sig_combine_cuda_associative(deg):
    dim = 3
    ...
    pysiglib.prepare_log_sig(dim, deg, 2)
    pysiglib.prepare_log_sig(dim, deg, 3)
    ls1 = pysiglib.log_sig(X1, deg, method=2).cuda()
    ls2 = pysiglib.log_sig(X2, deg, method=2).cuda()
    ls3 = pysiglib.log_sig(X3, deg, method=2).cuda()
...
@skip_no_cuda
@pytest.mark.parametrize("deg", range(1, 6))
def test_log_sig_combine_cuda_matches_cpu(deg):
    """Verify CUDA output matches CPU output exactly."""
    dim = 5
    X1 = torch.rand(size=(32, 100, dim), dtype=torch.float64)
    X2 = torch.rand(size=(32, 100, dim), dtype=torch.float64)
    X2 = torch.cat((X1[:, [-1], :], X2), dim=1)
    ...
    cpu_result = pysiglib.log_sig_combine(ls1, ls2, dim, deg)
    cuda_result = pysiglib.log_sig_combine(ls1.cuda(), ls2.cuda(), dim, deg)

    check_close(cpu_result, cuda_result)


@skip_no_cuda
@pytest.mark.parametrize("deg", range(1, 6))
def test_log_sig_combine_cuda_float32(deg):
    """Test with float32 dtype."""
    dim = 5
    X1 = torch.rand(size=(100, dim), dtype=torch.float32)
    X2 = torch.rand(size=(100, dim), dtype=torch.float32)
    X = torch.cat((X1, X2), dim=0)
    X2 = torch.cat((X1[[-1], :], X2), dim=0)
