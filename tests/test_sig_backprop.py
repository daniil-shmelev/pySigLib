...
def test_sig_backprop_random(device, deg, dtype):
    X = torch.rand(size=(100, 5), device=device, dtype=dtype)
    sig_derivs = torch.rand(size = (pysiglib.sig_length(5, deg),), device=device, dtype=dtype)
...
def test_batch_sig_backprop_random(device, deg):
    X = torch.rand(size=(100, 3, 2), dtype=torch.float64, device=device)
    sig_derivs = torch.rand(size=(100, pysiglib.sig_length(2, deg)), dtype=torch.float64, device=device)

    sig = pysiglib.sig(X, deg)

    sig_back1 = pysiglib.sig_backprop(X.clone(), sig.clone(), sig_derivs.clone(), deg)
    assert_device(sig_back1, device)
    sig_back2 = iisignature.sigbackprop(sig_derivs[:, 1:].cpu(), X.cpu(), deg)
    check_close(sig_back1, sig_back2)
