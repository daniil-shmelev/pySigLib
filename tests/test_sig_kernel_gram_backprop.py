...
def finite_difference(x1, x2, dyadic_order, time_aug = False, lead_lag = False):
    ...
    eps = 1e-10
    k = pysiglib.sig_kernel_gram(x1, x2, dyadic_order=dyadic_order, time_aug = time_aug, lead_lag = lead_lag)
    ...
    for i in range(length):
        for d in range(dim):
            x1_d = deepcopy(x1)
            x1_d[:,i,d] += eps
            k_d = pysiglib.sig_kernel_gram(x1_d, x2, dyadic_order=dyadic_order, time_aug = time_aug, lead_lag = lead_lag)
...
def test_sig_kernel_gram_backprop_batch(device, dyadic_order):
    ...
    d3, d4 = pysiglib.sig_kernel_gram_backprop(derivs, X, Y, dyadic_order=dyadic_order, left_deriv = True, right_deriv = True)
    assert_device(d3, device)
    assert_device(d4, device)
    d5, d6 = pysiglib.sig_kernel_gram_backprop(derivs, X, Y, dyadic_order=dyadic_order, left_deriv=True, right_deriv=True, max_batch = 2)
...
def test_sig_kernel_gram_backprop_batch_time_aug(device, dyadic_order):
    ...
    d3, d4 = pysiglib.sig_kernel_gram_backprop(derivs, X, Y, dyadic_order=dyadic_order, time_aug = True, left_deriv = True, right_deriv = True)
    assert_device(d3, device)
    assert_device(d4, device)
...
def test_sig_kernel_gram_backprop_batch_lead_lag(device, dyadic_order):
    ...
    d3, d4 = pysiglib.sig_kernel_gram_backprop(derivs, X, Y, dyadic_order=dyadic_order, lead_lag = True, left_deriv = True, right_deriv = True)
    assert_device(d3, device)
    assert_device(d4, device)
...
def test_sig_kernel_gram_backprop_batch_time_aug_lead_lag(device, dyadic_order):
    ...
    d3, d4 = pysiglib.sig_kernel_gram_backprop(derivs, X, Y, dyadic_order=dyadic_order, time_aug = True, lead_lag = True, left_deriv = True, right_deriv = True)
    assert_device(d3, device)
    assert_device(d4, device)
...
def test_sig_kernel_gram_backprop_batch_torch_api(device, dyadic_order):
    ...
    X.requires_grad_()
    Y.requires_grad_()
    K = pysiglib.torch_api.sig_kernel_gram(X, Y, dyadic_order=dyadic_order)
