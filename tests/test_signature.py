...
@pytest.mark.parametrize("device", DEVICES)
def test_signature_empty_primitives_match_default(device):
    X = FIXTURES["path"][0].astype(np.float64)
    X_dev = torch.tensor(X, device=device)
    primitives = torch.empty((0,), dtype=X_dev.dtype, device=device)

    expected = pysiglib.sig(X_dev, 4)
    actual = pysiglib.sig(X_dev, 4, primitives=primitives)
...
@pytest.mark.parametrize("device", DEVICES)
def test_signature_level2_primitives_single_segment(device):
    X = torch.tensor([[0.0], [3.0]], dtype=torch.float64, device=device)
    primitives = torch.tensor([2.0], dtype=torch.float64, device=device)

    actual = pysiglib.sig(X, 4, scalar_term=True, primitives=primitives)
...
@pytest.mark.parametrize("device", DEVICES)
def test_signature_level2_primitives_propagate_across_segments(device):
    X = torch.zeros((3, 1), dtype=torch.float64, device=device)
    primitives = torch.tensor([2.0], dtype=torch.float64, device=device)

    actual = pysiglib.sig(X, 3, scalar_term=True, primitives=primitives)
...
@pytest.mark.parametrize("device", DEVICES)
def test_signature_primitives_time_aug_zero_time_entries(device):
    X = torch.zeros((2, 1), dtype=torch.float64, device=device)
    primitives = torch.tensor([2.0], dtype=torch.float64, device=device)

    actual = pysiglib.sig(X, 2, time_aug=True, scalar_term=True, primitives=primitives)
...
def test_signature_primitives_validation_numpy():
    X = np.zeros((2, 2), dtype=np.float64)

    with pytest.raises(ValueError, match="1D"):
        pysiglib.sig(X, 2, primitives=np.zeros((2, 2), dtype=np.float64))

    with pytest.raises(ValueError, match="same dtype"):
        pysiglib.sig(X, 2, primitives=np.zeros((4,), dtype=np.float32))

    with pytest.raises(ValueError, match="prefix"):
        pysiglib.sig(X, 3, primitives=np.zeros((5,), dtype=np.float64))

    with pytest.raises(ValueError, match="lead_lag"):
        pysiglib.sig(X, 2, lead_lag=True, primitives=np.zeros((4,), dtype=np.float64))
