...
TEST(sigCoefCudaDoubleTest, TrivialTest) {
    ...
    EXPECT_EQ(0, err) << "sig_coef_cuda_d returned non-zero error code";
    for (uint64_t i = 0; i < true_.size(); ++i) {
        EXPECT_TRUE(std::abs(true_[i] - out[i]) < DOUBLE_EPSILON)
            << "Mismatch at index " + std::to_string(i);
    }
}

// Trivial test: single-point path should return zeros
TEST(sigCoefCudaDoubleTest, SinglePointPath) {
...
TEST(sigCoefCudaDoubleTest, FullSigCompare) {
    ...
    for (uint64_t i = 0; i < num_words; ++i) {
        std::string msg = "Mismatch at word " + std::to_string(i) +
            ": expected " + std::to_string(true_sig[i + 1]) +
            " got " + std::to_string(out[i]);
        EXPECT_TRUE(std::abs(true_sig[i + 1] - out[i]) < DOUBLE_EPSILON) << msg;
    }
}

// Batch test: verify sig_coef_cuda matches batch signature
TEST(sigCoefCudaDoubleTest, BatchFullSigCompare) {
    ...
    for (uint64_t b = 0; b < batch_size; ++b) {
        for (uint64_t i = 0; i < num_words; ++i) {
            ...
            std::string msg = "Batch " + std::to_string(b) + " word " + std::to_string(i) +
                ": expected " + std::to_string(expected) +
                " got " + std::to_string(actual);
            EXPECT_TRUE(std::abs(expected - actual) < DOUBLE_EPSILON) << msg;
        }
    }
}

// Prefixes test
TEST(sigCoefCudaDoubleTest, PrefixesTest) {
    ...
    for (uint64_t i = 0; i < out_size; ++i) {
        std::string msg = "Mismatch at index " + std::to_string(i) +
            ": expected " + std::to_string(true_[i]) +
            " got " + std::to_string(out[i]);
        EXPECT_TRUE(std::abs(true_[i] - out[i]) < DOUBLE_EPSILON) << msg;
    }
}

// Degree 0 word test: should return 1
TEST(sigCoefCudaDoubleTest, DegreeZeroWord) {
    ...
    EXPECT_EQ(0, err) << "sig_coef_cuda_d returned non-zero error code";
    EXPECT_TRUE(std::abs(1. - out[0]) < DOUBLE_EPSILON) << "Degree 0 word should return 1";
}

// Mixed degrees test
TEST(sigCoefCudaDoubleTest, MixedDegrees) {
