		...
		for (uint64_t batch_offset = 0; batch_offset < batch_size;) {
			...
			polysig_tile_forward_kernel_<T, FixedSize><<<
				batch_chunk.grid, 32U, smem>>>(
					gram, out, work, tables, rows, cols, length1, length2,
...
#define LAUNCH_POLYSIG_FIXED(size_value) \
	launch_polysig_tiles_<T, size_value>(gram, out, work.get(), tables, batch_size, \
	...
	default:
		launch_polysig_tiles_<T>(gram, out, work.get(), tables, batch_size,
...
extern "C" {
	...
	CUSIG_API int polysig_kernel_cuda_f(
		const float* gram, float* out, uint64_t batch_size, uint64_t dimension,
		uint64_t length1, uint64_t length2, uint64_t order, bool return_grid) noexcept {
		CUSIG_SAFE_CALL(polysig_kernel_cuda_<float>(
			gram, out, batch_size, dimension, length1, length2, order, return_grid));
	}

	CUSIG_API int polysig_kernel_cuda_d(
		const double* gram, double* out, uint64_t batch_size, uint64_t dimension,
		uint64_t length1, uint64_t length2, uint64_t order, bool return_grid) noexcept {
