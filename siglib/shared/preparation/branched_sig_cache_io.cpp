...
bool read_branched_sig_cache(
	...
	if (planar && loaded.basis_forest_offsets.empty()) {
		const BranchedSigCache canonical(dimension, max_nodes, true);
		if (canonical.total_length != loaded.total_length
			|| canonical.order_index != loaded.order_index)
			return false;
		loaded.basis_forest_data = canonical.basis_forest_data;
		loaded.basis_forest_offsets = canonical.basis_forest_offsets;
	}
	if (planar) {
