...
extern "C" {
	...
	*/
	...
	/**
	* @brief Sets the directory used by cpsig disk caches when ``use_disk=true``.
	...
	*/
	...
	/**
	* @brief Clear all cpsig in-memory caches, including log-signature, BCH, branched-signature, and prepared Volterra signature caches. This function is not thread safe.
	* 
	* @param use_disk If ``false``, will clear the cache from memory only.
        If ``true``, will also clear the shared cpsig disk cache directory.
	* @return Status code (0 = success).
	*/
