...
BranchedSigCoefPlan build_branched_sig_coef_plan_(
	const uint64_t* tree_indices,
	uint64_t num_indices,
	uint64_t data_dimension,
	uint64_t correction_max_level,
	...
	selected.reserve(num_indices * 4);
	pending.reserve(num_indices * 4);

	for (uint64_t i = 0; i < num_indices; ++i) {
		const uint64_t flat_idx = tree_indices[i];
		if (flat_idx >= cache.total_length)
			throw std::invalid_argument("branched_sig_coef tree index out of range");
		if (flat_idx != 0 && selected.insert(flat_idx).second)
			pending.push_back(flat_idx);
	}

	while (!pending.empty()) {
		const uint64_t flat_idx = pending.back();
		pending.pop_back();
		uint64_t pos = cache.coproduct_offsets[flat_idx - 1];
		const uint64_t pos_end = cache.coproduct_offsets[flat_idx];
		while (pos < pos_end) {
			const uint64_t num_forest = cache.coproduct_data[pos++];
			const uint64_t trunk_flat = cache.coproduct_data[pos++];
			if (trunk_flat != 0 && selected.insert(trunk_flat).second)
				pending.push_back(trunk_flat);
			for (uint64_t j = 0; j < num_forest; ++j) {
				const uint64_t forest_flat = cache.coproduct_data[pos++];
				if (forest_flat != 0 && selected.insert(forest_flat).second)
					pending.push_back(forest_flat);
			}
		}
	}

	std::vector<uint64_t> flat_indices(selected.begin(), selected.end());
	std::sort(flat_indices.begin(), flat_indices.end());

	std::unordered_map<uint64_t, uint64_t> local_index;
	local_index.reserve(flat_indices.size() + 1);
	local_index.emplace(0, 0);
	for (uint64_t i = 0; i < flat_indices.size(); ++i)
		local_index.emplace(flat_indices[i], i + 1);

	BranchedSigCoefPlan plan;
	const uint64_t plan_size = flat_indices.size() + 1;
	plan.target_indices.resize(num_indices);
	for (uint64_t i = 0; i < num_indices; ++i)
		plan.target_indices[i] = local_index.at(tree_indices[i]);

	plan.inv_tree_factorial.assign(plan_size, 1.0);
	plan.node_labels_offsets.assign(plan_size + 1, 0);
	plan.coproduct_offsets.assign(plan_size + 1, 0);
	for (uint64_t local = 1; local < plan_size; ++local) {
		const uint64_t flat_idx = flat_indices[local - 1];
		const uint64_t basis_idx = flat_idx - 1;
		plan.inv_tree_factorial[local] = cache.inv_tree_factorial[basis_idx];

		plan.node_labels_offsets[local] = plan.node_labels_data.size();
		const uint64_t label_start = cache.node_labels_offsets[basis_idx];
		const uint64_t label_end = cache.node_labels_offsets[basis_idx + 1];
		plan.node_labels_data.insert(plan.node_labels_data.end(),
			cache.node_labels_data.begin() + label_start,
			cache.node_labels_data.begin() + label_end);
		plan.node_labels_offsets[local + 1] = plan.node_labels_data.size();
		plan.max_nodes = std::max(plan.max_nodes, label_end - label_start);

		plan.coproduct_offsets[local] = plan.coproduct_data.size();
		uint64_t pos = cache.coproduct_offsets[basis_idx];
		const uint64_t pos_end = cache.coproduct_offsets[basis_idx + 1];
		while (pos < pos_end) {
			const uint64_t num_forest = cache.coproduct_data[pos++];
			const uint64_t trunk_flat = cache.coproduct_data[pos++];
			plan.coproduct_data.push_back(num_forest);
			plan.coproduct_data.push_back(local_index.at(trunk_flat));
			for (uint64_t j = 0; j < num_forest; ++j)
				plan.coproduct_data.push_back(local_index.at(cache.coproduct_data[pos++]));
		}
		plan.coproduct_offsets[local + 1] = plan.coproduct_data.size();
	}

	plan.leaf_indices.assign(cache.dimension, 0);
	if (cache.max_nodes >= 1) {
		for (uint64_t d = 0; d < cache.dimension; ++d) {
			const uint64_t leaf_flat = cache.order_index[1] + d + 1;
			const auto it = local_index.find(leaf_flat);
			if (it != local_index.end())
				plan.leaf_indices[d] = it->second;
		}
	}

	if (correction_max_level >= 2) {
		uint64_t correction_offset = 0;
		uint64_t level_size = data_dimension;
		for (uint64_t level = 2; level <= correction_max_level; ++level) {
			...
			for (uint64_t word_idx = 0; word_idx < level_size; ++word_idx) {
				...
				uint64_t power = level_size / data_dimension;
				for (uint64_t pos = level; pos > 0; --pos) {
					const uint64_t label = tmp / power;
					tmp -= label * power;
					if (pos > 1)
						power /= data_dimension;
					aug_word_idx = aug_word_idx * cache.dimension + label;
				}

				const uint64_t chain_flat = cache.chain_indices[
					cache.chain_index_offsets[level] + aug_word_idx];
				const auto it = local_index.find(chain_flat);
				if (chain_flat != 0 && it != local_index.end())
					plan.correction_indices.emplace_back(correction_offset + word_idx, it->second);
			}
...
FORCE_INLINE void sparse_linear_branched_sig_(
	const T* increment,
	T* out,
	const BranchedSigCoefPlan& plan
) {
	out[0] = static_cast<T>(1);
	const uint64_t plan_size = plan.inv_tree_factorial.size();
	const uint64_t* offsets = plan.node_labels_offsets.data();
	const uint8_t* labels = plan.node_labels_data.data();
	for (uint64_t local = 1; local < plan_size; ++local) {
		...
		out[local] = product * static_cast<T>(plan.inv_tree_factorial[local]);
	}
}
...
FORCE_INLINE void sparse_hopf_convolution_(
	...
	out[0] = X[0] * Y[0];
	for (uint64_t local = 1; local < plan.inv_tree_factorial.size(); ++local) {
		T value = X[local] * Y[0] + X[0] * Y[local];
		uint64_t pos = plan.coproduct_offsets[local];
		const uint64_t pos_end = plan.coproduct_offsets[local + 1];
		while (pos < pos_end) {
			const uint64_t num_forest = plan.coproduct_data[pos++];
			const uint64_t trunk = plan.coproduct_data[pos++];
			T term = Y[trunk];
			for (uint64_t j = 0; j < num_forest; ++j)
				term *= X[plan.coproduct_data[pos++]];
			value += term;
		}
		out[local] = value;
	}
...
FORCE_INLINE void sparse_butcher_product_inplace_(
	...
	const uint64_t plan_size = plan.inv_tree_factorial.size();
	const uint64_t* offsets = plan.coproduct_offsets.data();
	const uint64_t* data = plan.coproduct_data.data();
	for (uint64_t local = plan_size; local-- > 1;) {
...
FORCE_INLINE void build_sparse_local_log_(
	...
	std::fill(local_log, local_log + plan.inv_tree_factorial.size(), static_cast<T>(0));
	for (uint64_t d = 0; d < plan.leaf_indices.size(); ++d) {
		const uint64_t local = plan.leaf_indices[d];
		if (local != 0)
			local_log[local] = increment[d];
	}
	for (const auto& [correction_idx, local] : plan.correction_indices)
...
FORCE_INLINE void sparse_local_branched_sig_(
	...
	if (!has_correction) {
		sparse_linear_branched_sig_(increment, out, plan);
		return;
	}
	...
	const uint64_t plan_size = plan.inv_tree_factorial.size();
	build_sparse_local_log_(increment, correction, local_log, plan);
	std::fill(out, out + plan_size, static_cast<T>(0));
	out[0] = static_cast<T>(1);
	std::copy(local_log, local_log + plan_size, power);

	T inv_factorial = static_cast<T>(1);
	for (uint64_t k = 1; k <= plan.max_nodes; ++k) {
		inv_factorial /= static_cast<T>(k);
		for (uint64_t local = 1; local < plan_size; ++local)
			out[local] += inv_factorial * power[local];
		if (k < plan.max_nodes) {
			sparse_hopf_convolution_(power, local_log, next_power, plan);
...
FORCE_INLINE void sparse_branched_signature_with_buffers_(
	...
	sparse_local_branched_sig_(increment, out, local_log, power, next_power,
		correction, has_correction, plan);
	...
	for (uint64_t segment = 1; segment < length - 1; ++segment) {
		...
		sparse_local_branched_sig_(increment, temp, local_log, power, next_power,
			segment_correction, has_correction, plan);
		sparse_butcher_product_inplace_(out, temp, plan);
	}
}
...
FORCE_INLINE void sparse_butcher_product_deriv_(
	...
	std::copy(d_X + 1, d_X + plan_size, d_Y + 1);
...
FORCE_INLINE void sparse_hopf_convolution_deriv_(
	...
	for (uint64_t local = 1; local < plan.inv_tree_factorial.size(); ++local) {
		...
		while (pos < pos_end) {
			...
			T forest_product = static_cast<T>(1);
			for (uint64_t j = 0; j < num_forest; ++j)
				forest_product *= X[plan.coproduct_data[pos++]];
			d_Y[trunk] += d * forest_product;
			for (uint64_t k = 0; k < num_forest; ++k) {
				T partial = d * Y[trunk];
				for (uint64_t j = 0; j < num_forest; ++j) {
					if (j != k)
						partial *= X[plan.coproduct_data[forest_start + j]];
				}
				d_X[plan.coproduct_data[forest_start + k]] += partial;
			}
		}
	}
}
...
FORCE_INLINE void sparse_linear_bsig_deriv_to_increment_(
	...
	std::fill(increment_derivs, increment_derivs + plan.leaf_indices.size(), static_cast<T>(0));
	const uint64_t plan_size = plan.inv_tree_factorial.size();
	const uint64_t* offsets = plan.node_labels_offsets.data();
	const uint8_t* labels = plan.node_labels_data.data();
	for (uint64_t local = 1; local < plan_size; ++local) {
		...
		const T base = d * static_cast<T>(plan.inv_tree_factorial[local]);
		if (num_labels == 1) {
...
FORCE_INLINE void sparse_local_bsig_deriv_to_increment_(
	...
	if (!has_correction || plan.max_nodes <= 2) {
		sparse_linear_bsig_deriv_to_increment_(d_Y, increment, increment_derivs,
			prefix_products, plan);
		return;
	}

	const uint64_t plan_size = plan.inv_tree_factorial.size();
	build_sparse_local_log_(increment, correction, local_log, plan);
	std::copy(local_log, local_log + plan_size, powers);
	for (uint64_t k = 2; k <= plan.max_nodes; ++k)
		sparse_hopf_convolution_(powers + (k - 2) * plan_size, local_log,
			powers + (k - 1) * plan_size, plan);

	std::fill(power_derivs, power_derivs + plan.max_nodes * plan_size, static_cast<T>(0));
	std::fill(d_local_log, d_local_log + plan_size, static_cast<T>(0));
	T inv_factorial = static_cast<T>(1);
	for (uint64_t k = 1; k <= plan.max_nodes; ++k) {
		inv_factorial /= static_cast<T>(k);
		T* d_power = power_derivs + (k - 1) * plan_size;
		for (uint64_t local = 1; local < plan_size; ++local)
			d_power[local] += inv_factorial * d_Y[local];
	}

	for (uint64_t k = plan.max_nodes; k > 1; --k) {
		sparse_hopf_convolution_deriv_(powers + (k - 2) * plan_size, local_log,
			power_derivs + (k - 1) * plan_size,
			power_derivs + (k - 2) * plan_size, d_local_log, plan);
	}
	for (uint64_t local = 0; local < plan_size; ++local)
...
void branched_sig_coef_(
	...
	const uint64_t correction_max_level = validate_correction_len_(dimension, max_nodes, correction_len);
	...
	const auto& cache = get_branched_sig_cache(aug_dimension, max_nodes, planar);
	const auto plan = build_branched_sig_coef_plan_(tree_indices, num_indices, dimension,
		has_correction ? correction_max_level : 0, cache);
	const uint64_t plan_size = plan.inv_tree_factorial.size();
	...
	auto worker = [&](uint64_t start, uint64_t end) {
		...
		std::vector<T> state(plan_size);
		std::vector<T> temp(plan_size);
		std::vector<T> local_log(has_correction ? plan_size : 0);
		std::vector<T> power(has_correction ? plan_size : 0);
		std::vector<T> next_power(has_correction ? plan_size : 0);
		for (uint64_t batch = start; batch < end; ++batch) {
			...
			sparse_branched_signature_with_buffers_(path_obj, state.data(), increment.data(),
				...
				correction_segment_stride, has_correction, plan);
			T* out_ptr = out + batch * num_indices;
			for (uint64_t i = 0; i < num_indices; ++i)
				out_ptr[i] = state[plan.target_indices[i]];
		}
	};
...
void branched_sig_coef_backprop_(
	...
	auto worker = [&](uint64_t start, uint64_t end) {
		...
		std::vector<T> state(plan_size);
		std::vector<T> temp(plan_size);
		std::vector<T> state_derivs(plan_size);
		std::vector<T> local_derivs(plan_size);
		std::vector<T> increment_derivs(aug_dimension);
		std::vector<T> prefix_products(plan.max_nodes + 1);
		std::vector<T> local_log(has_correction ? plan_size : 0);
		std::vector<T> power(has_correction ? plan_size : 0);
		std::vector<T> next_power(has_correction ? plan_size : 0);
		std::vector<T> powers(has_correction && plan.max_nodes > 2 ? plan.max_nodes * plan_size : 0);
		std::vector<T> power_derivs(has_correction && plan.max_nodes > 2 ? plan.max_nodes * plan_size : 0);
		std::vector<T> d_local_log(has_correction && plan.max_nodes > 2 ? plan_size : 0);
		...
		for (uint64_t batch = start; batch < end; ++batch) {
			...
			for (uint64_t i = 0; i < num_indices; ++i) {
				const uint64_t target = plan.target_indices[i];
			...
			if (!lead_lag) {
				for (int64_t segment = static_cast<int64_t>(transformed_length) - 2;
					...
					sparse_local_branched_sig_(increment.data(), temp.data(), local_log.data(),
						power.data(), next_power.data(), segment_correction, has_correction, plan);
					if (segment > 0)
						sparse_butcher_uncombine_inplace_(state.data(), temp.data(), plan);
					if (segment > 0)
						sparse_butcher_product_deriv_(state.data(), temp.data(), state_derivs.data(),
							local_derivs.data(), plan);
					else
					...
					sparse_local_bsig_deriv_to_increment_(local_derivs.data(), increment.data(),
						...
						power_derivs.data(), d_local_log.data(), segment_correction, has_correction, plan);
				...
				for (int64_t segment = static_cast<int64_t>(transformed_length) - 2;
					...
					for (uint64_t d = 0; d < aug_dimension; ++d)
						increment[d] = p1[d] - p0[d];
					sparse_local_branched_sig_(increment.data(), temp.data(), local_log.data(),
						power.data(), next_power.data(), static_cast<const T*>(nullptr), false, plan);
					if (segment > 0)
						sparse_butcher_uncombine_inplace_(state.data(), temp.data(), plan);
					if (segment > 0)
						sparse_butcher_product_deriv_(state.data(), temp.data(), state_derivs.data(),
					...
					sparse_local_bsig_deriv_to_increment_(local_derivs.data(), increment.data(),
						...
						static_cast<const T*>(nullptr), false, plan);
...
extern "C" {
	...
	CPSIG_API int branched_sig_coef_f(const float* path, float* out, const uint64_t* tree_indices, uint64_t num_indices, uint64_t batch_size, uint64_t dimension, uint64_t length, uint64_t max_nodes, int n_jobs, bool time_aug, bool lead_lag, float end_time, bool planar, const float* correction, uint64_t correction_len, uint64_t correction_batch_stride, uint64_t correction_segment_stride) noexcept {
		SAFE_CALL(branched_sig_coef_<float>(path, out, tree_indices, num_indices, batch_size, dimension, length, max_nodes, n_jobs, time_aug, lead_lag, end_time, planar, correction, correction_len, correction_batch_stride, correction_segment_stride));
	}

	CPSIG_API int branched_sig_coef_d(const double* path, double* out, const uint64_t* tree_indices, uint64_t num_indices, uint64_t batch_size, uint64_t dimension, uint64_t length, uint64_t max_nodes, int n_jobs, bool time_aug, bool lead_lag, double end_time, bool planar, const double* correction, uint64_t correction_len, uint64_t correction_batch_stride, uint64_t correction_segment_stride) noexcept {
		SAFE_CALL(branched_sig_coef_<double>(path, out, tree_indices, num_indices, batch_size, dimension, length, max_nodes, n_jobs, time_aug, lead_lag, end_time, planar, correction, correction_len, correction_batch_stride, correction_segment_stride));
	}

	CPSIG_API int branched_sig_coef_backprop_f(const float* path, float* out, const float* coefs, const float* derivs, const uint64_t* tree_indices, uint64_t num_indices, uint64_t batch_size, uint64_t dimension, uint64_t length, uint64_t max_nodes, int n_jobs, bool time_aug, bool lead_lag, float end_time, bool planar, const float* correction, uint64_t correction_len, uint64_t correction_batch_stride, uint64_t correction_segment_stride) noexcept {
		SAFE_CALL(branched_sig_coef_backprop_<float>(path, out, coefs, derivs, tree_indices, num_indices, batch_size, dimension, length, max_nodes, n_jobs, time_aug, lead_lag, end_time, planar, correction, correction_len, correction_batch_stride, correction_segment_stride));
	}

	CPSIG_API int branched_sig_coef_backprop_d(const double* path, double* out, const double* coefs, const double* derivs, const uint64_t* tree_indices, uint64_t num_indices, uint64_t batch_size, uint64_t dimension, uint64_t length, uint64_t max_nodes, int n_jobs, bool time_aug, bool lead_lag, double end_time, bool planar, const double* correction, uint64_t correction_len, uint64_t correction_batch_stride, uint64_t correction_segment_stride) noexcept {
		SAFE_CALL(branched_sig_coef_backprop_<double>(path, out, coefs, derivs, tree_indices, num_indices, batch_size, dimension, length, max_nodes, n_jobs, time_aug, lead_lag, end_time, planar, correction, correction_len, correction_batch_stride, correction_segment_stride));
	}
