...
from ._array import ArrayT as Array
...
from .param_checks import check_dtype, check_n_jobs, check_non_neg, check_type
...
def extract_branched_sig_coef(
    ...
    *,
    ...
    if bsig.ndim < 1:
        raise ValueError("bsig must have at least rank 1")
    check_dtype(bsig, "bsig")
...
def prepare_branched_sig_coef(
    ...
    *,
    ...
    _, degree, tree_data = _branched_coef_data(
        trees, augmented_dimension, planar)
    tree_data_tensor = np.asarray(tree_data, dtype=np.uint64)
    tree_data_ptr = pointer(tree_data_tensor, c_uint64)
