...
from ._storage import add_at, pair_indices, pointer
...
def branched_gram_deriv(
    ...
    if err_code:
        raise Exception(
            "Error in pysiglib.branched_sig_kernel_backprop: "
...
def branched_sig_kernel_gram_backprop(
    ...
    *,
    ...
    ld = (
        xp.zeros(path1.shape, dtype=path1.dtype, device=device(path1)) if left_deriv else None
    )
    rd = (
        xp.zeros(path2.shape, dtype=path2.dtype, device=device(path2)) if right_deriv else None
    )
