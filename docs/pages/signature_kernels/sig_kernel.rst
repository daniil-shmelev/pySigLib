...
The polynomial solver supports CPU NumPy and CPU Torch forward computations.
It does not support CUDA tensors, ``return_grid=True``, or the reverse-mode
interfaces in ``pysiglib.torch_api`` and ``pysiglib.jax_api``. Its native
solver workspace is linear in the number of increments of the second path,
but the increment Gram matrix constructed by the Python API remains quadratic
in the two path lengths.
