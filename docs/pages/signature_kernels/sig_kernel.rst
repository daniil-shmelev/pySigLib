...
The polynomial solver supports CPU NumPy, Torch, and JAX computations and
CUDA Torch and JAX computations, including reverse mode. With
``return_grid=True``, it returns the scalar kernel value at every original
path vertex. Reverse mode retains the incoming boundary coefficients for each
cell, or regenerates them when no saved state is available. Solver workspace
is linear in the path lengths, but the increment Gram matrix and saved reverse
state remain quadratic in the two path lengths.
